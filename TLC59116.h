  ...
  public:
    ...
    */
    TLC59116& group_blink(
      ...
      ) { return group_blink(0xFFFF,(int)blink_period,(int)on_ratio); }
    ...
    TLC59116& group_blink(word bit_pattern, double blink_length_secs, double on_percent=50.0) { // convenience % & len
      // on_percent is 05..99.61%, blink_length is 0.041secs (24.00384hz) to 10.54secs (.09375hz) in steps of .04166secs
        // FIXME:  It's messy, but to remove ambiguity we need to refer to group_blink(word, int, int) -- hence the extra (int) cast
        return group_blink(
        bit_pattern, 
        (int) (byte) int(blink_length_secs/0.041666667 + .0001),
        (int) (byte) int(on_percent * 256.0/100.0)
        );
      }
...
class TLC59116Manager {
  ...
  public:
    ...
    */
    ...
    // TODO: get by I2C address, which we might could do because 0x60 > 15...
  ...
  private:
    ...
    TLC59116Manager& operator=(const TLC59116Manager&); // undefined        // why is this private!
