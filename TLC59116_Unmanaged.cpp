...
void TLC59116_Unmanaged::describe_iref(byte* registers) {
    ...
    Serial.print(F("IREF_Register: "));Serial.println(value, BIN);
...
byte TLC59116_Unmanaged::i_out_d(byte CM, byte HC, byte D, int Rext) { 
  ...
  //       Vref    VG = (1+HC) * ((1 + D/64)/4)
  return ((1.26 * ((1.0 + HC) * (1.0 + D/64.0) / 4.0)) / (double)Rext) * (CM ? 15 : 5) *1000;
  }
...
byte TLC59116_Unmanaged::best_iref(byte ma, int Rext) {
  ...
  if (ma <= 5 || ma < 1575/Rext) {       // MINIMUM CONDITION
    HC=0; CM=0; D=0;
    actual = i_out_d(CM, HC, D, Rext);
    // printf("Too small, actual %dma HC %d, CM %d, D %03o\n",actual, HC, CM, D);
    }
  else if (ma >= 120 || ma > 28202.3/Rext) {   // MAXIMUM CONDITION
    HC=1; CM=1; D=0b111111;
    actual = i_out_d(CM, HC, D, Rext);
    // printf("Too big, actual %dma HC %d, CM %d, D %03o\n",actual, HC, CM, D);
    }
  else {           // NORMAL
    if (ma <= 4725/Rext) {               // SMALL, LOW CURRENT
      HC=0; CM=0; D = (2.560*ma*Rext)/63.0 - 64;
      actual = i_out_d(CM, HC, D, Rext);
      // printf("segment 1, actual %dma HC %d, CM %d, D %03o\n",actual, HC, CM, D);
      }
    else if (ma <= 14101.2/Rext) {      // MID SIZE
      HC=0; CM=1; D = (2.560*ma*Rext)/189.0 - 64;
      actual = i_out_d(CM, HC, D, Rext);
      // printf("segment 2, actual %dma HC %d, CM %d, D %03o\n",actual, HC, CM, D);
      }
    if (ma >= 3150/Rext) {          // COULD BE LOW OR HIGH, FIND BEST
      if (ma <= 9449.98/Rext) {
        HCp=1; CMp=0; Dp = (1.280*ma*Rext )/63.0 - 64;  // FIXED: THIS USED TO CLOBBER HC AND CM
    ...
    // In the conditions where Rext truncates your current the above calculations
    // will result in too large a D.  This then gets truncated by reverse_cc
    // and results in a low setting (off by 2 when ready by milliamps() ).
  if (D > 63 ) {
    D = 63;           //Prevent D from getting truncated by reverse_cc    
    }
 
  byte iref = 0;
  // CC (D reversed)
  iref = reverse_cc(D);  //NOTE:  (D) a byte made from a truncated float
  iref |= HC << 6 | CM << 7;
  ...
  return iref; // FIXME: ..CRASHES on line 226 on first power up...and the flags
  }
